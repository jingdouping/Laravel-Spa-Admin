...
const SecondCategoryDatatable = () => {
  ...
  useEffect(()=>{
    loginaxios.get('/api/view-secondcategory').then(res=>{
      if(res.status === 200){
  ...
  const deleteCategory = (e,id) =>{
    ...
    if(result){
      ...
      loginaxios.post(`/api/delete-secondcategory/${id}`).then(res=>{
    ...
    renderCell:(params)=>{
      return(
        <div className='cellAction' >
          <Link to={`/secondcategories/${params.row.id}`} style={{textDecoration:'none'}}>
  ...
  return (
    <div className='datatable'>
      <div className='datatableTitle'>
        ...
        <Link to='/secondcategories/new' className='link'>
          新規作成
        </Link>
      </div>
